...
def export_revolved_step(profile_points, output_path, angle_deg=360, use_spline=False):
    ...
    try:
        ...
        if len(profile_points) < 3:
            raise ValueError(
                f"profile has only {len(profile_points)} points after dedup"
            )
...
# ============================================================================
# Aftertopo: strip-shell + BFS-driven overhang support
# ----------------------------------------------------------------------------
# Per-mold algorithm:
#   1. Build a thin strip along the cavity boundary, offset by
#      band_width = H * band_ratio (default 1/100) into the mold body.
#   2. retained = mold_mask ∩ strip_mask  (carve everything else away)
#   3. BFS from the build-platform row, inside retained_mask only, with
#      moves = (build_direction, left, right). Retained pixels that are not
#      visited form the overhang set.
#   4. For each overhang connected component, fill a rectangle spanning the
#      cluster x-range, from the cluster's floor-far side all the way to the
#      floor row. Clip to the original mold mask.
#   5. final = retained ∪ support → extract external contours → revolve → STEP.
#
# Build orientation (symmetric treatment of the two molds):
#   lower mold: floor = pixel bottom row (physical y = bottom_y);
#               build move = pixel -y (= physical +y).
#   upper mold: floor = pixel top row    (physical y = top_y);
#               build move = pixel +y (= physical -y).
# ============================================================================


def build_lower_strip_profile(lower_edge_phys, width_mm, band_width_mm):
    """
    Closed polygon for the strip along the lower cavity edge, offset downward
    by band_width_mm. x range matches the lower-mold profile (0..width_mm).
    """
    lower = lower_edge_phys[np.argsort(lower_edge_phys[:, 0])]
    right_x = float(width_mm)
    band = float(band_width_mm)

    points = []
    if lower[0, 0] > 0:
        points.append([0.0, float(lower[0, 1])])
    points.extend([p.tolist() for p in lower])
    if lower[-1, 0] < right_x:
        points.append([right_x, float(lower[-1, 1])])
    points.append([right_x, float(lower[-1, 1]) - band])
    if lower[-1, 0] < right_x:
        points.append([float(lower[-1, 0]), float(lower[-1, 1]) - band])
    points.extend([[float(p[0]), float(p[1]) - band] for p in lower[::-1]])
    if lower[0, 0] > 0:
        points.append([0.0, float(lower[0, 1]) - band])

    return remove_consecutive_duplicates(np.array(points, dtype=float))


def build_upper_strip_profile(upper_edge_phys, width_mm, x_expand_mm,
                               band_width_mm):
    """
    Closed polygon for the strip along the upper cavity edge, offset upward
    by band_width_mm. x extends to width + x_expand (matches upper mold).
    """
    upper = upper_edge_phys[np.argsort(upper_edge_phys[:, 0])]
    right_x = float(width_mm) + float(x_expand_mm)
    band = float(band_width_mm)
    ...
    if upper[0, 0] > 0:
        points.append([0.0, float(upper[0, 1])])
    points.extend([p.tolist() for p in upper])
    if upper[-1, 0] < right_x:
        points.append([right_x, float(upper[-1, 1])])
    points.append([right_x, float(upper[-1, 1]) + band])
    if upper[-1, 0] < right_x:
        points.append([float(upper[-1, 0]), float(upper[-1, 1]) + band])
    points.extend([[float(p[0]), float(p[1]) + band] for p in upper[::-1]])
    if upper[0, 0] > 0:
        points.append([0.0, float(upper[0, 1]) + band])

    return remove_consecutive_duplicates(np.array(points, dtype=float))


def bfs_overhang_from_floor(retained_mask, floor_side):
    """
    BFS within retained_mask starting from the floor row. Unreachable
    retained pixels form the overhang set.
    """
    h, w = retained_mask.shape
    visited = np.zeros_like(retained_mask, dtype=bool)
    queue = deque()

    if floor_side == "bottom":
        seed_row = h - 1
        build_move = (-1, 0)   # pixel -y = physical +y
    elif floor_side == "top":
        seed_row = 0
        build_move = (1, 0)    # pixel +y = physical -y
    else:
        raise ValueError(f"Unknown floor_side: {floor_side}")

    moves = (build_move, (0, -1), (0, 1))

    for x in range(w):
        if retained_mask[seed_row, x]:
            visited[seed_row, x] = True
            queue.append((seed_row, x))

    while queue:
        y, x = queue.popleft()
        for dy, dx in moves:
            ny, nx = y + dy, x + dx
            if 0 <= ny < h and 0 <= nx < w and retained_mask[ny, nx] \
                    and not visited[ny, nx]:
                visited[ny, nx] = True
                queue.append((ny, nx))

    overhang = retained_mask & ~visited
    return visited, overhang


def fill_support_columns(overhang_mask, floor_side, canvas_shape,
                          min_cluster_area_px=1, connectivity=8):
    """
    For each connected component in overhang_mask, fill a rectangular column
    spanning the cluster's x-range, from the cluster's floor-far side down
    (or up) to the floor row.
    """
    h, w = canvas_shape
    support_mask = np.zeros((h, w), dtype=bool)

    overhang_u8 = overhang_mask.astype(np.uint8)
    num_labels, labels, stats, _ = cv2.connectedComponentsWithStats(
        overhang_u8, connectivity=connectivity
    )
    ...
    for label in range(1, num_labels):
        ...
        if floor_side == "bottom":
            support_mask[y_max:h, x_min:x_max + 1] = True
        else:
            support_mask[0:y_min + 1, x_min:x_max + 1] = True
    ...
    return support_mask, clusters


def mask_to_all_physical_profiles(mask, grid, simplify_eps=1.0, min_area_px=10,
                                    close_kernel=3, close_iters=1,
                                    dedup_tol_mm=1e-3):
    """Extract every external contour from mask and convert to physical polygons.

    Morphological closing is applied first to fill pixel-level gaps and
    smooth sharp ridges that can produce self-intersecting polygons.
    """
    ...
    for c in contours:
        if cv2.contourArea(c) < min_area_px:
            continue
        c_s = cv2.approxPolyDP(c, simplify_eps, True) if simplify_eps > 0 else c
        pts = c_s.squeeze().astype(float)
        if pts.ndim != 2 or len(pts) < 3:
            continue
        phys = remove_consecutive_duplicates(
            bfs_pixel_to_physical(pts, grid),
            tol=dedup_tol_mm,
        )
        if len(phys) >= 3:
...
def save_aftertopo_preview(mold_mask, retained_mask, overhang_mask,
                            support_mask, final_mask, bfs_grid,
                            label, output_prefix):
    """
    Overlay showing:
      gray     - original mold body (for reference)
      orange   - retained strip, reachable from floor
      red      - overhang pixels in strip (unreachable from floor)
      green    - added support columns (only new pixels, outside retained)
      black    - final mold outline
    """
    canvas_h, canvas_w = bfs_grid["shape"]
    img = np.full((canvas_h, canvas_w, 3), 255, dtype=np.uint8)

    img[mold_mask] = (240, 240, 240)
    retained_ok = retained_mask & ~overhang_mask
    img[retained_ok] = (80, 160, 230)
    img[overhang_mask] = (60, 60, 220)
    sup_only = support_mask & ~retained_mask
    img[sup_only] = (130, 200, 130)
    ...
    cv2.drawContours(img, contours, -1, (30, 30, 30), 2)

    cv2.rectangle(img, (8, 8), (210, 118), (255, 255, 255), -1)
    cv2.rectangle(img, (8, 8), (210, 118), (80, 80, 80), 1)
    cv2.putText(img, f"aftertopo: {label}", (14, 28),
                cv2.FONT_HERSHEY_SIMPLEX, 0.5, (30, 30, 30), 1)
    cv2.rectangle(img, (14, 36), (30, 48), (240, 240, 240), -1)
    cv2.putText(img, "mold body", (36, 47),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (60, 60, 60), 1)
    cv2.rectangle(img, (14, 54), (30, 66), (80, 160, 230), -1)
    cv2.putText(img, "retained (reach)", (36, 65),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (60, 60, 60), 1)
    cv2.rectangle(img, (14, 72), (30, 84), (60, 60, 220), -1)
    cv2.putText(img, "overhang", (36, 83),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (60, 60, 60), 1)
    cv2.rectangle(img, (14, 90), (30, 102), (130, 200, 130), -1)
    cv2.putText(img, "support fill", (36, 101),
                cv2.FONT_HERSHEY_SIMPLEX, 0.4, (60, 60, 60), 1)

    write_png(f"{output_prefix}_{label}_aftertopo_preview.png", img)
...
def main():
    ...
    # --- aftertopo (strip-shell + BFS overhang support) ---
    parser.add_argument("--no-aftertopo", action="store_true",
                        help="Skip the strip-shell aftertopo step")
    parser.add_argument("--band-ratio", type=float, default=1.0 / 100.0,
                        help="Strip band thickness as fraction of image height (default 1/100)")
    parser.add_argument("--min-cluster-area", type=int, default=1,
                        help="Ignore overhang clusters smaller than this pixel area")
    ...
    # =========================================================================
    # AFTERTOPO: strip-shell along cavity edge + BFS overhang support columns
    # =========================================================================
    aftertopo_ready = False
    if not args.no_aftertopo:
        band_width_mm = float(args.band_ratio) * float(args.height)

        # --- LOWER MOLD ---
        lower_strip_profile = build_lower_strip_profile(
            lower_edge_phys, args.width, band_width_mm,
        )
        lower_mold_mask = rasterize_profile(lower_bfs_profile, bfs_grid)
        lower_strip_mask = rasterize_profile(lower_strip_profile, bfs_grid)
        lower_retained = lower_mold_mask & lower_strip_mask
        # Reuse directional_bfs so seeds cover both the floor row and the
        # left column (rotation axis). For a single-connected strip touching
        # x=0, BFS from the axis spreads along the whole strip -> no overhang
        # -> pure hollow shell is exported.
        lower_visited_aft = directional_bfs(lower_retained, "lower")
        lower_overhang = lower_retained & ~lower_visited_aft
        lower_support, lower_clusters = fill_support_columns(
            lower_overhang, "bottom", bfs_grid["shape"],
            min_cluster_area_px=args.min_cluster_area,
        )
        lower_support = lower_support & lower_mold_mask
        lower_final_mask = lower_retained | lower_support
        lower_aftertopo_profiles = mask_to_all_physical_profiles(
            lower_final_mask, bfs_grid, simplify_eps=args.simplify,
        )

        # --- UPPER MOLD ---
        upper_strip_profile = build_upper_strip_profile(
            upper_edge_phys, args.width, x_expand_mm, band_width_mm,
        )
        upper_mold_mask = rasterize_profile(upper_bfs_profile, bfs_grid)
        upper_strip_mask = rasterize_profile(upper_strip_profile, bfs_grid)
        upper_retained = upper_mold_mask & upper_strip_mask
        upper_visited_aft = directional_bfs(upper_retained, "upper")
        upper_overhang = upper_retained & ~upper_visited_aft
        upper_support, upper_clusters = fill_support_columns(
            upper_overhang, "top", bfs_grid["shape"],
            min_cluster_area_px=args.min_cluster_area,
        )
        upper_support = upper_support & upper_mold_mask
        upper_final_mask = upper_retained | upper_support
        upper_aftertopo_profiles = mask_to_all_physical_profiles(
            upper_final_mask, bfs_grid, simplify_eps=args.simplify,
        )

        l_orig = int(lower_mold_mask.sum())
        l_final = int(lower_final_mask.sum())
        l_save = 100.0 * (l_orig - l_final) / max(l_orig, 1)
        u_orig = int(upper_mold_mask.sum())
        u_final = int(upper_final_mask.sum())
        u_save = 100.0 * (u_orig - u_final) / max(u_orig, 1)

        print("\n--- aftertopo (strip-shell + overhang support) ---")
        print(f"  band width       : {band_width_mm:.4f} mm"
              f"  (ratio={args.band_ratio})")
        print(f"  lower: retained_px={int(lower_retained.sum())},"
              f" overhang_px={int(lower_overhang.sum())},"
              f" support_px={int(lower_support.sum())},"
              f" clusters={len(lower_clusters)},"
              f" contours={len(lower_aftertopo_profiles)},"
              f" save={l_save:.1f}%")
        print(f"  upper: retained_px={int(upper_retained.sum())},"
              f" overhang_px={int(upper_overhang.sum())},"
              f" support_px={int(upper_support.sum())},"
              f" clusters={len(upper_clusters)},"
              f" contours={len(upper_aftertopo_profiles)},"
              f" save={u_save:.1f}%")

        upper_aftertopo_path = output_with_suffix(output_path, "_upper_cavity_aftertopo")
        lower_aftertopo_path = output_with_suffix(output_path, "_lower_cavity_aftertopo")

        if lower_aftertopo_profiles:
            if len(lower_aftertopo_profiles) == 1:
                export_revolved_step(
                    lower_aftertopo_profiles[0],
                    lower_aftertopo_path,
                    args.angle,
                )
            else:
                export_multiple_revolved_step(
                    lower_aftertopo_profiles,
                    lower_aftertopo_path,
                    args.angle,
                )
            print(f"exported: {lower_aftertopo_path}")
        else:
            print("warning: no lower aftertopo contour generated")

        if upper_aftertopo_profiles:
            if len(upper_aftertopo_profiles) == 1:
                export_revolved_step(
                    upper_aftertopo_profiles[0],
                    upper_aftertopo_path,
                    args.angle,
                )
            else:
                export_multiple_revolved_step(
                    upper_aftertopo_profiles,
                    upper_aftertopo_path,
                    args.angle,
                )
            print(f"exported: {upper_aftertopo_path}")
        else:
            print("warning: no upper aftertopo contour generated")

        aftertopo_ready = True
    ...
    if not args.no_preview:
        ...
        if aftertopo_ready:
            save_aftertopo_preview(
                lower_mold_mask, lower_retained, lower_overhang,
                lower_support, lower_final_mask, bfs_grid,
                "lower", output_prefix,
            )
            print(f"preview: {output_prefix}_lower_aftertopo_preview.png")
            save_aftertopo_preview(
                upper_mold_mask, upper_retained, upper_overhang,
                upper_support, upper_final_mask, bfs_grid,
                "upper", output_prefix,
            )
            print(f"preview: {output_prefix}_upper_aftertopo_preview.png")
